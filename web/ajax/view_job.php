...
if ($_POST['scene'] && $_POST['shot'] && $_POST['updateid']) {
		...
		if ($_POST['action'] == "duplicate") {
			...
			$query="INSERT INTO jobs (
					...
					pre_render_action,post_render_action,status,progress_status,progress_remark,priority,
					lastseen,created_by,last_edited_by
				) VALUES (
					'$scene','$shot','$start','$end','$project',
					'$start','$chunks','$filetype','$rem','$config',
					'$pre_render_action','$post_render_action','active','$progress_status','$progress_remark','$priority',
			...
			$queryqq="UPDATE jobs SET start='$start', end='$end', filetype='$filetype', rem='$rem', config='$config', pre_render_action='$pre_render_action', post_render_action='$post_render_action', chunks='$chunks', priority='$priority', progress_status='$progress_status', progress_remark='$progress_remark', lastseen=NOW(), last_edited_by='$_SESSION[user]' WHERE id=$job_id;";
			mysql_query($queryqq);
			if ($directstart == "true"){
